...
"""Training entrypoint for affordvla single-task VLA training."""
...
# Local Modules
from affordvla.dataloader import build_dataloader
from affordvla.model.framework import build_framework
from affordvla.training.trainer_utils.config_tracker import wrap_config
from affordvla.training.trainer_utils.trainer_tools import (
    TrainerUtils,
...
def prepare_data(cfg, accelerator, output_dir) -> DataLoader:
    ...
    data_mix = cfg.datasets.vla_data.get("data_mix", cfg.datasets.vla_data.dataset_py)
    ...
    def _train_step(self, batch_vla):
        with self.accelerator.accumulate(self.model):
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                total_loss = output_dict["total_loss"]  # total_loss = action_loss + mask_loss
        ...
        # total_loss / action_loss / mask_loss
        step_metrics = {"action_dit_loss": total_loss.item()}
        if output_dict.get("action_loss") is not None:
            step_metrics["action_loss_raw"] = output_dict["action_loss"].item()
        if output_dict.get("mask_loss") is not None:
            step_metrics["mask_loss"] = output_dict["mask_loss"].item()
        return step_metrics
