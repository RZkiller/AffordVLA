...
class RegionPooling(nn.Module):
    ...
    def extract_region_feature(self, region_feature_map, region_masks, original_dtype, return_dtype, spatial_shapes=None):
        ...
        for img_id, (region_feature_map_i, region_masks_list_i) in enumerate(zip(region_feature_map, region_masks)):
            ...
            if len(region_masks_list_i) != 0:
                ...
                cur_non_zero_pos = [rand_sample_repeat((m.nonzero() / ori_image_wh), self.num_sample_point) for m
                                    in region_masks_list_i] # torch.Size([5, 256, 2])
                # [num_mask, num_sample_point, 2]
                cur_non_zero_pos = torch.stack(cur_non_zero_pos) # torch.Size([5, 256, 2])

                # Use explicit spatial shape if provided, otherwise assume square
                if spatial_shapes is not None:
                    h, w = spatial_shapes[img_id]
                else:
                    h = w = int(math.sqrt(region_feature_map_i.shape[0])) # num of patches
                c = region_feature_map_i.shape[-1] # 2048

                dup_region_feature_map_i = region_feature_map_i.reshape(h, w, c).permute(2, 0, 1) # torch.Size([C, h, w])
                dup_region_feature_map_i = dup_region_feature_map_i.unsqueeze(0).repeat(cur_non_zero_pos.shape[0], 1, 1,
                                                                                        1) # torch.Size([num_mask, C, h, w])
                ...
                # debug: 如果有的affordance mask全0, 则过 pooling 后得到全零 embedding，显式表达"该视角无 affordance 区域"
                # Zero out features for all-zero masks (no affordance region)
                for mask_idx, m in enumerate(region_masks_list_i):
                    if m.sum() == 0:
                        region_feature_i[mask_idx] = 0.0
    ...
    def forward(self, feature_map, region_masks, original_dtype, return_dtype, spatial_shapes=None):
        ...
        # debug
        # print("region_feature_list check!!!", type(region_feature_list), len(region_feature_list), region_feature_list[0].shape)
