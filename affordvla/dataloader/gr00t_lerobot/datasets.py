...
from affordvla.dataloader.gr00t_lerobot.video import get_all_frames, get_frames_by_timestamps

from affordvla.dataloader.gr00t_lerobot.embodiment_tags import EmbodimentTag
from affordvla.dataloader.gr00t_lerobot.schema import (
    DatasetMetadata,
    DatasetStatisticalValues,
    LeRobotModalityMetadata,
    LeRobotStateActionMetadata,
)
from affordvla.dataloader.gr00t_lerobot.transform import ComposedModalityTransform
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ---- Dataloader Mask Loading: Affordance mask root directory ---- #
        mask_root = data_cfg.get("mask_root_dir", None) if data_cfg else None
        self._mask_root_dir = Path(mask_root) if mask_root else None
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        if stats_path.exists():
            try:
                ...
                print(
                    f"[RANK {os.environ.get('RANK', 'NA')}] " f"Failed to load dataset statistics ({e}), rebuilding..."
                )
                le_statistics_by_mode = None
    ...
    def _get_tasks(self) -> pd.DataFrame:
        ...
        if self._lerobot_version == "v2.0":
            ...
            df = df.reset_index()  # Convert index to a column, usually named 'index'
            df = df.rename(columns={"index": "task"})  # Rename column 'index' to 'task'
    ...
    def _get_trajectories(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        # v3.0
            ...
            # The save index info should be directly readable here
            return np.array(trajectory_ids), np.array(trajectory_lengths)
    ...
    def get_step_data(self, trajectory_id: int, base_index: int) -> dict:
        ...
        # ---- Dataloader Mask Loading: Extract affordance mask paths from parquet columns ---- #
        if (
            self._mask_root_dir is not None
            and self.curr_traj_data is not None
            and "affordance_mask.primary" in self.curr_traj_data.columns
        ):
            data["affordance_mask.primary"] = self.curr_traj_data["affordance_mask.primary"].iloc[base_index]
            data["affordance_mask.wrist"] = self.curr_traj_data["affordance_mask.wrist"].iloc[base_index]
    ...
    def get_trajectory_data_lerobot_v3(self, trajectory_id: int) -> pd.DataFrame:
        ...
        if self.curr_traj_id == trajectory_id and self.curr_traj_data is not None:
            ...
            from_timestamp = self.trajectory_ids_to_metadata[trajectory_id][
                "videos/observation.images.wrist/from_timestamp"
            ]
    ...
    # Pack data into training sample format: convert video frames to PIL Images and concatenate action arrays
    def _pack_sample(self, data: dict) -> dict:
        ...
        # ---- Dataloader Mask Loading: Load affordance masks from PNG files ---- #
        if "affordance_mask.primary" in data:
            prim_mask_path = self._mask_root_dir / data["affordance_mask.primary"]
            wrist_mask_path = self._mask_root_dir / data["affordance_mask.wrist"]
            assert prim_mask_path.exists(), f"Primary mask not found: {prim_mask_path}"
            assert wrist_mask_path.exists(), f"Wrist mask not found: {wrist_mask_path}"
            prim_mask = torch.from_numpy(
                (np.array(Image.open(prim_mask_path).convert("L")) > 127).astype(np.uint8)
            )
            wrist_mask = torch.from_numpy(
                (np.array(Image.open(wrist_mask_path).convert("L")) > 127).astype(np.uint8)
            )
            # Order must match all_images = prim_images + wrist_views
            sample["affordance_mask"] = [prim_mask, wrist_mask]
